...
import { Bell, BookOpenText, Briefcase, Building2, CheckCheck, ClipboardList, FileText, GraduationCap, LayoutDashboard, LogOut, Menu, MessageSquare, Settings, Shield, User as UserIcon, Users, X } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen text-slate-900">
            ...
            <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
                <button className="fixed inset-0 bg-slate-950/50" onClick={() => setSidebarOpen(false)} />
                <aside className="fixed inset-y-0 left-0 flex w-80 flex-col border-r border-white/10 bg-slate-950 text-white shadow-2xl">
                    <div className="flex h-20 items-center justify-between border-b border-white/10 px-5">
                        ...
                        <button onClick={() => setSidebarOpen(false)} className="rounded-md p-2 text-slate-400 hover:bg-white/10"><X size={20} /></button>
                    </div>
                    <Navigation mobile />
                    <div className="mx-4 mb-4 rounded-3xl border border-white/10 bg-white/5 p-4">
                        <div className="flex items-center gap-3">
                            <div className={`flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br ${roleAccent} text-slate-950`}><Shield size={16} /></div>
                            <div>
            ...
            <aside className="fixed inset-y-0 left-0 hidden w-80 flex-col border-r border-white/10 bg-slate-950 text-white lg:flex">
                ...
                <div className="border-t border-white/10 p-5">
                    <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-4">
                        <div className="flex items-center gap-3">
                            <div className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br ${roleAccent} text-slate-950`}><BookOpenText size={17} /></div>
                            <div>
                                <p className="text-sm font-semibold">{roleLabel}</p>
            ...
            <div className="lg:pl-80">
                <header className="sticky top-0 z-40 border-b border-white/10 bg-slate-950/75 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8">
                    <div className="flex min-h-16 items-center justify-between gap-4 rounded-3xl border border-white/10 bg-white/8 px-4 py-3 text-white">
                        <button onClick={() => setSidebarOpen(true)} className="rounded-2xl border border-white/10 bg-white/10 p-2.5 text-white transition hover:bg-white/15 lg:hidden"><Menu size={20} /></button>
                        <div className="min-w-0 flex-1 px-3">{header}</div>
                        <div className="flex items-center gap-2">
                            <div className="relative" ref={notifRef}>
                                <button onClick={() => setNotifOpen(!notifOpen)} className="relative rounded-2xl border border-white/10 bg-white/10 p-2.5 text-white transition hover:bg-white/15">
                                    <Bell size={18} />
                                    {unreadCount > 0 && (
                                ...
                                {notifOpen && (
                                    <div className="absolute right-0 mt-2 w-96 rounded-2xl border border-slate-200 bg-white shadow-xl z-50">
                                        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {unreadCount > 0 && (
                                                    <button onClick={markAllAsRead} className="flex items-center gap-1 text-xs text-teal-600 hover:text-teal-700">
                                        ...
                                        <div className="max-h-80 overflow-y-auto">
                                            {notifications.length === 0 ? (
                                                ...
                                                <button
                                                    ...
                                                    className={`w-full border-b border-slate-50 px-4 py-3 text-left transition hover:bg-slate-50 ${!n.est_lu ? 'bg-teal-50/50' : ''}`}
                                                >
                                                    <div className="flex items-start gap-3">
                                                        {!n.est_lu && <div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-teal-500" />}
